...
def getMostPopular(topic):
    apiKey = '3401ee66ceb65d0033cd3148bf9da9d5:7:72986752'
    urlString = 'http://api.nytimes.com/svc/mostpopular/v2/mostviewed/' + topic + '/1.json?api-key=' + apiKey
...
def getSteamData(appID):
    apiKey = '2EEB7E7F692F81F531BA1DA43DFA5DAB'
    urlString = 'http://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/?appid=' + appID + '&count=1&maxlength=1000&format=json'
    urlResponseFile = urllib2.urlopen(urlString)
...
def getXKCD(comicNumString):
    if comicNumString == '0':
        urlString = 'http://xkcd.com/info.0.json'
    else:
        urlString = 'http://xkcd.com/' + comicNumString + '/info.0.json'
...
def index():
    ...
    if request.method == 'POST':
        ...
        if request.form.get("cookieOperation") == 'retrieve_cookie':
            ...
            cookieColour = database3210.decryptString(currentCookie)
            ...
            colourCookieString = database3210.encryptString(currentColour)
...
def searchPortal(newsDesk, input):
    ...
    if request.method == 'GET':
        ...
        outString = database3210.checkCache(newsDesk, queryInput);
...
def searchPortalBlank():
    ...
    if request.method == 'GET':
        # Get search results from either NYT or the database
        outString = database3210.checkCache('Technology', queryInput);
